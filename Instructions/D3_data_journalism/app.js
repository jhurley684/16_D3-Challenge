...
var chosenYAxis = "obesityLow";


// function used for updating x-scale var upon click on axis label
// does the scaleLinear based on the chosen axis
function xScale(healthData, chosenXAxis) {
  // create scales
  var xLinearScale = d3.scaleLinear()
    .domain([d3.min(healthData, d => d[chosenXAxis]) * 0.8,  //min val based on chosen axis
      d3.max(healthData, d => d[chosenXAxis]) * 1.2          //max val based on chosen axis
    ])
    .range([0, width]);
...
function updateToolTip(chosenXAxis, circlesGroup) {
  ...
  var toolTip = d3.tip()
    ...
    .offset([80, -60])
    .html(function(d) {
      return (`${d.obesityHigh}<br>${label} ${d[chosenXAxis]}`);
    });
